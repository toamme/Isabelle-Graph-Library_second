...
"weight x y = weight x' y' \<Longrightarrow>
 abstract_real_map (p_lookup pot) x = abstract_real_map (p_lookup pot) x'
\<Longrightarrow> abstract_real_map (p_lookup pot) y = abstract_real_map (p_lookup pot) y' \<Longrightarrow>
edge_slack pot x y = edge_slack pot x' y'"
"weight x y = weight x' y' \<Longrightarrow>
 abstract_real_map (p_lookup pot) y = abstract_real_map (p_lookup pot) x'
\<Longrightarrow> abstract_real_map (p_lookup pot) x = abstract_real_map (p_lookup pot) y' \<Longrightarrow>
edge_slack pot x y = edge_slack pot x' y'"
  by(auto simp add: edge_slack_def)

definition "build_tight_graph pmap =
           g_iterate_g G_impl (\<lambda>  (u, v) tights. if (edge_slack pmap u v = 0) then g_insert u v tights
                                      else tights) g_empty"
...
(\<lambda> v S. if abstract_real_map (p_lookup pmap) v > 0 then vset_insert v S
        else S) vset_empty"
...
lemma naive_primal_dual_impl_same:
  ...
  apply(induction rule: naive_primal_dual.pinduct[OF assms(1)])
  apply(subst naive_primal_dual.psimps)
  by(auto split: match_or_witness_set.split simp add: naive_primal_dual_impl.simps)
...
lemma naive_primal_dual_ret_condI:
 "\<And> tights bads M. tights = build_tight_graph pmap \<Longrightarrow>
     bads = find_bads pmap \<Longrightarrow> find_matching_or_witness tights bads = match M 
...
lemma naive_primal_dual_call_condI:
 "\<And> tights bads S NS. tights = build_tight_graph pmap \<Longrightarrow>
     bads = find_bads pmap \<Longrightarrow> find_matching_or_witness tights bads =  witness S NS
...
lemma naive_primal_dom_call:
     "naive_primal_dual_dom (naive_primal_dual_call pmap) \<Longrightarrow>
      naive_primal_dual_call_cond pmap \<Longrightarrow>
      naive_primal_dual_dom pmap"
...
lemma naive_primal_dual_simps:
    ...
    "naive_primal_dual_call_cond pmap \<Longrightarrow>naive_primal_dual_dom pmap \<Longrightarrow>
          naive_primal_dual pmap = naive_primal_dual (naive_primal_dual_call pmap)"
  subgoal
...
"distinct xs \<Longrightarrow> (\<And> x. x \<in> set xs \<Longrightarrow>  prod.swap x \<notin> set xs \<or> fst x = snd x) \<Longrightarrow>
 pair_list_distinct xs"
and  pair_list_extended_distinctE:
"pair_list_distinct xs \<Longrightarrow>
  (distinct xs \<Longrightarrow> (\<And> x. x \<in> set xs \<Longrightarrow>  prod.swap x \<notin> set xs \<or> fst x = snd x) \<Longrightarrow> P)
 \<Longrightarrow> P"
  by(auto simp add: pair_list_distinct_def)

lemma pair_list_distinct_front[simp]: "pair_list_distinct (x#xs) = 
...
and find_matching_or_witness:
 "\<And> E A B R M. bipartite (g_to_set E) A B \<Longrightarrow> g_invar E \<Longrightarrow> vset_invar R
  \<Longrightarrow> find_matching_or_witness E R = match M \<Longrightarrow>
   graph_matching (g_to_set E) (g_to_set M) \<and> vset_set R \<subseteq> Vs (g_to_set M)
   \<and> g_invar M"
 "\<And> E A B R S NS. bipartite (g_to_set E) A B \<Longrightarrow> g_invar E \<Longrightarrow> vset_invar R
  \<Longrightarrow> find_matching_or_witness E R = witness S NS \<Longrightarrow>
   vset_set S \<subseteq> vset_set R \<and> vset_invar S \<and> vset_invar NS \<and>
     (vset_set S \<subseteq> A \<or> vset_set S \<subseteq> B) \<and>
     vset_set NS = Neighbourhood (g_to_set E) (vset_set S) \<and>
...
lemma basic_graph_props: "Vs G \<subseteq> LR" "LR = L \<union> R" "L \<inter> R = {}" "bipartite G L R"
                         "x1 \<in> L \<Longrightarrow> {x1, x2} \<in> G \<Longrightarrow> x2 \<in> R"
                         "y1 \<in> R \<Longrightarrow> {y1, y2} \<in> G \<Longrightarrow> y2 \<in> L"
                         "a1 \<in> R \<Longrightarrow> a2 \<in> R \<Longrightarrow> {a1, a2} \<in> G \<Longrightarrow> False"
                         "b1 \<in> L \<Longrightarrow> b2 \<in> L \<Longrightarrow> {b1, b2} \<in> G \<Longrightarrow> False"
                         "x1 \<notin> L \<Longrightarrow> x1 \<in> Vs G \<Longrightarrow> x1 \<in> R"
                         "x1 \<notin> R \<Longrightarrow> x1 \<in> Vs G \<Longrightarrow> x1 \<in> L"
                         "finite L" "finite R" "finite LR"
  using G_props(3,5,7,6) by(force simp add: LR_def L_def R_def bipartite_def Vs_def)+
...
lemma in_G_sym_slack: "{u, v} \<in> G \<Longrightarrow> edge_slack p u v = edge_slack p v u"
...
(\<lambda> v. if v \<in> L then Max (insert 0 {weight v u | u. {u, v} \<in> G})
     else 0)"
...
proof-
  ...
  have abstract_real_map_after_its_is:
     ...
     (\<lambda> v. if v \<in> L then Max (insert 0 {weight v u | u. (u, v) \<in> set es \<or> (v, u) \<in> set es})
           else 0)" 
     if  "pair_list_distinct es" "set_of_pair ` set es \<subseteq> G"
    using that
  proof(induction es)
    case Nil
    then show ?case by(auto simp add: p_map.map_empty abstract_real_map_empty)
  next
    case (Cons e es)
    ...
    have es_weight_sym: "(u, v) \<in> set (e#es) \<Longrightarrow> weight u v = weight v u" for u v
      by (metis Cons.prems(2) image_subset_iff set_of_pair_applied_to_pair sym_weights)
    have Max_is: " Max ({0} \<union> {weight v u |  u. (u, v) \<in> set (e # es) \<or>
      ...
      apply(subst linorder_class.Max_insert[symmetric])
      apply(subst pair_weight_fold)+ 
      apply simp
      apply(rule finite_subset[of _ "pair_weight ` (set es \<union> prod.swap ` set es)"])
      by(auto split: if_split 
             intro!: arg_cong[of _ _ Max ]
              intro: exI[of _ "snd e"]
           simp add: split_pairs  es_weight_sym)
    show ?case  
      apply(subst foldr.simps, subst o_apply, subst it_def)
  ...
  have es_weight_sym: "(u, v) \<in> set (es) \<Longrightarrow> weight u v = weight v u" for u v
    by (metis G_props(2) es_prop(2) imageI set_of_pair_applied_to_pair sym_weights)
  have same_weights: "{weight v u |u. (u, v) \<in> set es \<or> (v, u) \<in> set es}
                      = {weight v u |u. {u, v} \<in> G}" for v
...
proof-
  ...
  have g_set_is:"g_to_set (foldr it es g_empty) = 
       {{u, v} | u v. (u, v) \<in> set es \<and> edge_slack pmap u v = 0}"
    by(induction es)
  ...
  "g_to_set (build_tight_graph pmap) = {{u, v} | u v. {u, v} \<in> G \<and> edge_slack pmap u v = 0}"
    ...
    by (auto simp add:  build_tight_graph_def it_def[symmetric] es_prop(3) G_props(2) set_of_pair_def )
       (subst (asm) doubleton_eq_iff | fastforce intro!: exI[of "\<lambda> x. \<exists> y. {a, b } = {x, y} \<and> (x, y) \<in> set es \<and> edge_slack pmap x y = 0" for a b,
                 OF exI])+
qed
...
lemma find_epsilon_props:
  ...
  shows "find_\<epsilon> pmap S = 
    Min (insert PInfty ({edge_slack pmap u v | u v. 
               vset_isin S u \<and> {u, v} \<in> G \<and> edge_slack pmap u v \<noteq> 0} \<union>
               {abstract_real_map (p_lookup pmap) v | v. vset_isin S v}))"
proof-
  define it_e where "it_e = (\<lambda>(u, v) val.
              let sl = edge_slack pmap u v
              in if sl \<noteq> 0 \<and> ereal sl < val \<and> (vset_isin S u \<or> vset_isin S v) then ereal sl
                 else val)"
  ...
  obtain vs where vs_prop: "distinct vs" "set vs = vset_set S" "vset_iterate_real S it_v PInfty = foldr it_v vs PInfty"
    using vset_iterations(1)[OF assms(1), of it_v PInfty] by auto
    have h1:"ereal (edge_slack pmap b a) \<notin> (\<lambda>x. ereal (edge_slack pmap (fst x) (snd x))) ` set es \<Longrightarrow>
         (a, b) \<in> set es \<Longrightarrow> False" for a b
      ...
      by fastforce force
    have finites2: "finite
        ({ereal (edge_slack pmap u v) |u v.
          vset_isin S u \<and> {u, v} \<in> set_of_pair ` set es \<and> edge_slack pmap u v \<noteq> 0})"
      apply(rule finite_subset[of _ "(\<lambda> (u, v). ereal (edge_slack pmap u v)) ` set (es)", OF _ finite_imageI])
      using  h1
      by(auto simp add: set_of_pair_def doubleton_eq_iff case_prod_beta 
         intro:  edge_slack_cong) force+
  have min_edges:"foldr it_e es PInfty = Min (insert PInfty {ereal (edge_slack pmap u v) |u v.
  ...
  proof(induction es)
    ...
    have finites: "finite
     {ereal (edge_slack pmap u v) |u v.
      ...
      apply(rule finite_subset[of _ "(\<lambda> (u, v). ereal (edge_slack pmap u v)) ` set (e#es)", OF _ finite_imageI])
      using Cons(2)[of x y] h1
      by (auto simp add: e_split set_of_pair_def doubleton_eq_iff case_prod_beta 
         intro:  edge_slack_cong) force+
    have finites2: "finite
        ({ereal (edge_slack pmap u v) |u v.
          vset_isin S u \<and> {u, v} \<in> set_of_pair ` set es \<and> edge_slack pmap u v \<noteq> 0})"
    ...
    have in_e_same_slack: "(a, b) \<in> set es \<Longrightarrow> edge_slack pmap a b = edge_slack pmap b a"
    ...
    have IH_applied: "foldr it_e es PInfty =
     Min ({PInfty} \<union>
      ...
      next
        case 3
        have h1: "\<forall>u. vset_isin S u \<longrightarrow>
        (\<forall>v. foldr it_e es \<infinity> = ereal (edge_slack pmap u v) \<longrightarrow>
             ...
             edge_slack pmap u v = 0) \<Longrightarrow>
    \<not> ereal (edge_slack pmap x y) < foldr it_e es \<infinity> \<Longrightarrow> foldr it_e es \<infinity> = \<infinity>"
          using IH_applied[simplified]
          by (smt (verit, best) Min_in finite_insert finites2 insert_iff insert_not_empty
                mem_Collect_eq)
        have h2: "\<forall>u. vset_isin S u \<longrightarrow>
        (\<forall>v. foldr it_e es \<infinity> = ereal (edge_slack pmap u v) \<longrightarrow>
             {u, v} \<noteq> set_of_pair (x, y) \<and> {u, v} \<notin> set_of_pair ` set es \<or>
             edge_slack pmap u v = 0) \<Longrightarrow>
           edge_slack pmap x y = 0 \<Longrightarrow> foldr it_e es \<infinity> = \<infinity>"
          using IH_applied[simplified] 
          by (smt (verit, del_insts) Min_in finite_insert finites2 insert_compr insert_not_empty
              mem_Collect_eq)
        have h3: "\<forall>u. vset_isin S u \<longrightarrow>
        (\<forall>v. foldr it_e es \<infinity> = ereal (edge_slack pmap u v) \<longrightarrow>
             {u, v} \<noteq> set_of_pair (x, y) \<and> {u, v} \<notin> set_of_pair ` set es \<or>
             edge_slack pmap u v = 0) \<Longrightarrow>
    \<not> vset_isin S x \<Longrightarrow> \<not> vset_isin S y \<Longrightarrow> foldr it_e es \<infinity> = \<infinity>"
          using IH_applied[simplified]
          by (smt (verit, ccfv_SIG) Min_in finite_insert finites2 insert_iff insert_not_empty
              mem_Collect_eq)
        show ?case 
        proof(rule, goal_cases)
          ...
          then show ?case 
            ...
            by(auto intro: exI[of "\<lambda> u. \<exists>v. _ x y = _ u v \<and> _ u v", OF exI, of _ _ x y, OF conjI]
                 simp add: set_of_pair_applied_to_pair doubleton_eq_iff 
        ...
        next
          ...
          have a2: "\<lbrakk>(b, a) \<in> set es; vset_isin S a; edge_slack pmap a b \<noteq> 0\<rbrakk>
            ...
            by (smt (verit, best) Min.bounded_iff empty_def finite_insert finites2 image_eqI insert_commute
                insert_iff mem_Collect_eq order_refl set_of_pair_applied_to_pair)
          have h2: " ereal (edge_slack pmap x y) < foldr it_e es \<infinity> \<Longrightarrow> vset_isin S u \<Longrightarrow>
                     edge_slack pmap u v \<noteq> 0 \<Longrightarrow> {u, v} = set_of_pair (a, b) \<Longrightarrow> (a, b) \<in> set es \<Longrightarrow> edge_slack pmap x y \<le> edge_slack pmap u v" for a b u v
          ...
          show ?case
            using h2
            by(auto simp add: Let_def  intro: h1 h2 h3 h4 h5 h6)
        qed
      qed
    qed simp
    have vs_min_is:"foldr it_v vs PInfty = 
    ...
    proof(induction vs)
     ...
     "Min ({\<infinity>} \<union> {ereal (abstract_real_map (p_lookup pmap) va) |va. va = v \<or> va \<in> set vs})
        ...
        by(force intro!: arg_cong[of _ _ Min])+
      have less_Min:
         "Min ({\<infinity>} \<union> {ereal (abstract_real_map (p_lookup pmap) v) |v. v \<in> set vs})
          \<le> ereal (abstract_real_map (p_lookup pmap) v)" if
          "abstract_real_map (p_lookup pmap) v \<ge> abstract_real_map (p_lookup pmap) va"
          "va \<in> set vs"  for va
        using that(2) 
    ...
    unfolding find_\<epsilon>_def it_e_def[symmetric] it_v_def[symmetric] es_prop(3) vs_prop(3)
       es_prop(2)[simplified G_props(2), symmetric] min_edges vs_prop(2)[symmetric]
       vs_min_is
    by(auto simp add: linorder_class.Min_Un[symmetric] vset.set_isin[OF assms(1)] vs_prop(2)[symmetric])
qed
...
proof-
  define it where "it = (\<lambda>v S. if 0 < abstract_real_map (p_lookup pmap) v then vset_insert v S else S)"
  ...
  show "vset_invar (find_bads pmap)"
       "vset_set (find_bads pmap) = {v | v. v \<in> LR \<and> abstract_real_map (p_lookup pmap) v > 0}"
    by (auto simp add: vset_invar  find_bads_def it_def[symmetric] vs(2,3) set_is lefts_and_rights_props(2))
qed
...
"epsilon_multiples (\<epsilon>::real) f X \<Longrightarrow>
 ((\<And> x. x \<in> X \<Longrightarrow> \<exists> n::nat. f x = n * \<epsilon>) \<Longrightarrow> P) \<Longrightarrow> P"
and  epsilon_multiplesD:
"epsilon_multiples (\<epsilon>::real) f X \<Longrightarrow> x \<in> X \<Longrightarrow> \<exists> n::nat. f x = n * \<epsilon>"
  by(auto simp add: epsilon_multiples_def)

lemma epsilon_multiples_sum: "finite X \<Longrightarrow> epsilon_multiples \<epsilon> f X \<Longrightarrow>
                         epsilon_multiples \<epsilon> id {sum f X}"
...
lemma finite_g_applied_double:"finite {f x y | x y. P x y} \<Longrightarrow> finite {g (f x y) | x y. P x y}"
  by (smt (verit, best) Collect_cong finite_image_set)

lemma finite_g_applied_single:"finite {f x | x . P x} \<Longrightarrow> finite {g (f x) | x. P x}"
  by (smt (verit, best) Collect_cong finite_image_set)

lemma real_of_ereal_of_Min_or_ereal:
"finite A \<Longrightarrow> A \<noteq> {} \<Longrightarrow> real_of_ereal (Min (ereal ` A)) = Min A"
  by(auto simp add: ereal_Min[symmetric])

lemma Collect_double_f_to_single: "{ g (f x y) | x y. P x y} = {g ff | ff. \<exists> x y. ff = f x y \<and> P x y}"
  by auto

lemma Collect_single_f_to_single: "{ g (f x) | x. P x} = {g ff | ff. \<exists> x. ff = f x \<and> P x}"
  by auto

lemma Collect_vset_set:"vset_invar S \<Longrightarrow> Collect (vset_isin S) = vset_set S"
  ...
  shows   "p_invar (naive_primal_dual_call pmap)"
    ...
    and  "\<exists> \<alpha> > 0. sum  (abstract_real_map (p_lookup (naive_primal_dual_call pmap))) LR + \<alpha> =
            ...
            \<and> (epsilon_multiples \<epsilon> w G \<and> epsilon_multiples \<epsilon> (potential pmap) LR \<longrightarrow>
                epsilon_multiples \<epsilon> id {\<alpha>} \<and> epsilon_multiples \<epsilon> (potential (naive_primal_dual_call pmap)) LR)" (is ?long_thesis)
proof-
 define tights where "tights = build_tight_graph pmap"
  ...
  have pmap''_is_call:"naive_primal_dual_call pmap = pmap''"
    ...
    by(auto simp add: naive_primal_dual_call_def pmap''_def pmap'_def \<alpha>_def bads_def tights_def Let_def)
  ...
  have S_NS_in_LR: "vset_set S \<subseteq> L  \<Longrightarrow> vset_set NS \<subseteq> R" 
    ...
    using tights_bipartite S_NS_props(3) Neighbourhood_bipartite_left apply blast
  ...
  have eps_eral_is:"\<alpha> = real_of_ereal (Min ({PInfty} \<union>
  ...
  have finite_slacks: "finite {edge_slack pmap u v |u v. vset_isin S u \<and> {u, v} \<in> G \<and> edge_slack pmap u v \<noteq> 0}"
    using finiteG
    by(force intro: finite_subset[of _ "{edge_slack pmap u v |u v. {u, v} \<in> G}"] 
          simp add: image_two_Collect intro:  finite_pairs_of_finite_set_set)
  ...
  have alpha_in: "\<alpha> \<in> {(edge_slack pmap u v) |u v. vset_isin S u \<and> {u, v} \<in> G \<and> edge_slack pmap u v \<noteq> 0} \<union>
      {(potential pmap v) |v. vset_isin S v}"
    unfolding alpha_is
    using finite_slacks finite_ys  S_non_empty' 
    by (intro Min_in) force+
  hence alpha_witness: "(\<exists> u v.  vset_isin S u \<and> {u, v} \<in> G \<and> edge_slack pmap u v \<noteq> 0 \<and> \<alpha> = (edge_slack pmap u v))
  ...
  have alpha_less: 
   " vset_isin S u \<Longrightarrow> {u, v} \<in> G \<Longrightarrow> edge_slack pmap u v \<noteq> 0 \<Longrightarrow> \<alpha> \<le> edge_slack pmap u v"
  ...
  obtain vs where vs:"distinct vs" "set vs = vset_set S"
    ...
    by auto
  have p_invar_it:"p_invar pmap \<Longrightarrow>
        p_invar (foldr (\<lambda>v. p_update v (f v + x)) vs pmap)" for x pmap vs f
    by(induction vs)(auto simp add: p_map.invar_update)
  have p_invar_not_in_vs_no_change:"p_invar pmap \<Longrightarrow> v \<notin> set vs \<Longrightarrow>
  ...
  proof(rule exI[of _ "(card (vset_set S) - card (vset_set NS)) * \<alpha>"], goal_cases)
      ...
      proof(rule disjE[OF alpha_witness], goal_cases)
        ...
        have ex_x:"\<exists>x. \<alpha> = \<epsilon> * real x"
          if "\<epsilon> * real xb + \<epsilon> * real xc \<noteq> \<epsilon> * real xa" "\<alpha> + \<epsilon> * real xa = \<epsilon> * real xb + \<epsilon> * real xc"
           ...
           for xa xb xc
          using that
          apply(intro exI[of _ "(xb + xc - xa)"])
          by (smt (verit, ccfv_threshold) distrib_left le_add_same_cancel2 mult_left_mono nle_le nless_le
              of_nat_add of_nat_diff of_nat_le_iff of_nat_less_0_iff right_diff_distrib'
              zero_le_mult_iff)
        show ?case
    ...
    show "epsilon_multiples \<epsilon> id {real (card (vset_set S) - card (vset_set NS)) * \<alpha>}"
      using epsilon_multiplesD[OF epsilon_multiple_alpha, simplified, OF refl] 
      by(auto intro!: epsilon_multiplesI)(metis of_nat_mult)
    show "epsilon_multiples \<epsilon> (potential pmap'') LR"
    proof(rule  epsilon_multiplesI, goal_cases)
      case (1 v)
      have "0 < \<epsilon> * real xa \<Longrightarrow>
         \<epsilon> * real xa \<le> \<epsilon> * real x \<Longrightarrow>
         \<alpha> = \<epsilon> * real xa \<Longrightarrow>\<exists>xb. \<epsilon> * real x = \<epsilon> * real xa + \<epsilon> * real xb" for x xa
        by(smt (verit, ccfv_SIG) mult_less_cancel_left_disj nless_le of_nat_0 of_nat_diff
            of_nat_less_0_iff of_nat_less_iff right_diff_distrib')
      moreover have "\<exists>xb. \<epsilon> * real x + \<epsilon> * real xa = \<epsilon> * real xb" for x xa
        by (metis distrib_left of_nat_add)
      ultimately show ?case 
...
qed (auto simp add: basic_graph_props(1,13))
  ...
  proof(rule epsilon_multiplesI, goal_cases)
    ...
    then obtain u where u_prop: "(potential init_potential v = weight v u \<and> {u, v} \<in> G)
      ...
      by (auto simp add: init_potential_props(2) ) meson
    show ?case
  ...
  proof(induction n arbitrary: pmap rule: less_induct)
    ...
    next
      ...
      have "\<lbrakk> 0 < \<epsilon> * real n1; \<epsilon> * real n1 + \<epsilon> * real n3 = \<epsilon> * real n2; 0 < \<epsilon>\<rbrakk>
        ...
        by (smt (verit, best) mult_left_mono of_nat_less_iff)
  ...
  next
    ...
    next
      ...
      have finitess: "finite {uu. \<exists>u v r. uu = r \<and> real_of_rat r = weight u v / \<epsilon> \<and> {u, v} \<in> G}"
        apply(rule finite_subset[of _ "{r. \<exists>e . real_of_rat r = w e / \<epsilon> \<and> e \<in> G}"])
        subgoal
          using w_weight_cong by force
        subgoal
          apply(subst bij_betw_finite[of real_of_rat "{r. \<exists>e. real_of_rat r = w e / \<epsilon> \<and> e \<in> G}"
                         "{r. \<exists>e.  r = w e / \<epsilon> \<and> e \<in> G}"])
          subgoal
            apply(auto simp add: bij_betw_def image_def intro!: inj_onI )
             apply (metis of_rat_eq_iff)
            by (metis False basic_graph_props(4) bipartite_edgeE nonzero_mult_div_cancel_right w_weight_cong
                weights_scaled_rationals)
          by (simp add: finiteG)
        done
      obtain d where d:
