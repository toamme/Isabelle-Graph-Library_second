          ...
          proof(intro even_alt_pathI, goal_cases)
            ...
            next
              ...
              have "path (insert {target, u'} E) (butlast (quot'.refine C M (p @ [u'])))"
                by (metis append_butlast_last_id butlast.simps(1) path_in_old path_pref)
              ...
              proof(rule ccontr, goal_cases)
                ...
                hence "u' \<in> set (butlast (quot'.refine C M (p @ [u'])))"
                  by (meson edge_not_in_edges_in_path)
                hence "\<not> distinct (quot'.refine C M (p @ [u']))"
                  using One 
                  by (metis append_butlast_last_id butlast.simps(1) distinct_butlast not_distinct_conv_prefix penultimate_is
                      target_not_u')
                then show ?case 
          ...
          proof(intro even_alt_pathI, goal_cases)
            ...
            next
              ...
              proof(rule ccontr, goal_cases)
                ...
                hence "\<not> distinct (quot'.refine C M (p @ [u']))"
                  using Two 
                  by (metis Nil_tl distinct.simps(2) empty_iff empty_set hd_Cons_tl)
                then show ?case 
                  using path_in_old by auto
              qed
...
proof(rule,  goal_cases)
  ...
  have even_len:"even (length (stem @ C))"
    by (metis assms(1) even_add length_append match_blossomD(3) match_blossom_stem_even
        odd_cycle_even_verts)
  have "butlast (tl (stem @ C)) = tl stem @ butlast C"
    by (metis assms(1,3) butlast_append match_blossomD(3) odd_cycle_nempty tl_append2)
  ...
  then show ?case 
   ...
   next
     ...
     next
       case 3
       hence "ua \<in> Vs E" 
         by (meson assms(4) edges_are_Vs subsetD)
        ...
        next
          ...
          proof(elim disjE, goal_cases)
            ...
            then obtain y' where "{x', y'} \<in>  (quotG M) \<lbrakk>D\<rbrakk>"
              by (metis (no_types, lifting) "2" Diff_iff M_in_E doubleton_quot finite_E finite_dbl_finite_verts
                  finite_quot graph_invar_graph_inter_Vs graph_invar_no_edge_no_vertex rev_finite_subset x(2))
            then show ?case
...
proof-
  ...
  have u_not_in_D:"u \<notin> \<Union> \<D>" 
    by (metis good_quot_map(1) edmonds_gallaiD(2) subsetD)
