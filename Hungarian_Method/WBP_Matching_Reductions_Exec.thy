...
(*TODO MOVE*)
context Pair_Graph_Specs
begin

definition "from_list es = foldl (\<lambda> G (x, y). add_edge G x y) empty es"

lemmas [code] = from_list_def

lemma finite_graph_empty: "finite_graph \<emptyset>\<^sub>G"
  by (simp add: adjmap.map_empty finite_graph_def)
...
"\<lbrakk>adjmap_inv G; has_neighb_set G x\<rbrakk> \<Longrightarrow> has_neighb_set (add_edge G u v) x"
  by(auto simp add: add_edge_def has_neighb_set_def adjmap.map_update split: option.split)

lemma add_edge_no_change:
 "\<lbrakk>adjmap_inv G; u \<noteq> v\<rbrakk> \<Longrightarrow> lookup (add_edge G u u') v = lookup G v"
  by (simp add: add_edge_def adjmap.map_update option.case_eq_if)
...
proof-
  ...
  have "vset_inv (foldr insert vs V) \<and>
    ...
    by(induction vs)
      (auto simp add: vset.set.invar_insert vset.set.set_insert assms)
